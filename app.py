...
class LogImporter:
    ...
    def parse(cls, raw: bytes, filename: str, label: str, desc_field: str = "") -> tuple:
        ...
        for i, row in enumerate(rows):
            # FIX v6.1: Cap at 300 before parsing remaining rows to avoid
            # holding large datasets fully in memory before truncation.
            if len(events) >= 300:
                warnings.append(
                    f"Stopped importing at 300 events; file contained {len(rows)} rows. "
...
def build_html_report(
    ...
    for r in recommendations:
        ...
        recs_html += f"""<div class="rec-card" style="border-left-color:{c}">
          ...
          <p style="color:#94a3b8;font-size:.83rem;line-height:1.7;margin:4px 0">{_html.escape(r['body'])}</p>
          <div class="rec-fix">
            <span style="font-size:.75rem;font-weight:700;text-transform:uppercase;
              letter-spacing:1px;color:{c};margin-right:6px">FIX →</span>
            <span style="font-size:.83rem;color:#64748b">{_html.escape(r['fix'])}</span>
          </div></div>\n"""
...
with st.sidebar:
    ...
    _import_label_opts = list(LogImporter.CATEGORY_MAP.keys()) + ["auto-detect"]
    import_label = st.selectbox(
        "Label imported events as",
        _import_label_opts,
        index=_import_label_opts.index("auto-detect"),  # FIX v6.1: computed, not hardcoded
        label_visibility="collapsed",
    )

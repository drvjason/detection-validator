...
def load_framework():
    ...
    try:
        ...
        raise ImportError(
            f"Failed to execute detection_validator.py ({resolved_path}): {exc}"
        ) from exc
...
try:
    ...
    st.error(f"❌ Unexpected error loading detection_validator.py — {type(e).__name__}: {e}")
    st.stop()
...
with st.sidebar:
    ...
    # Demo loader — sets session state before the text_area renders so
    # the widget shows the rule text immediately on the same rerun.
    _DEMO_RULE = """\
...
level: medium"""
    ...
    # Resolve default: demo session-state takes priority.
    _demo_default = st.session_state.get("_demo_rule", "")
    ...
    # Clear demo state once the user edits the text themselves.
    if _demo_default and rule_text != _demo_default:
        del st.session_state["_demo_rule"]

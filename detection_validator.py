...
class TelemetryGenerator:
    ...
    def _random_timestamp(self, days_back: int = 7) -> str:
        ...
        is an anti-pattern. Now uses datetime.utcnow() directly.
        """
...
class TestRunner:
    ...
    def get_metrics(self) -> dict:
        ...
        g   = self.grading
        ev_r = evasion_resistance if evasion_resistance is not None else 1.0
        score = (
            f1   * g.f1_weight
            + ev_r * g.evasion_weight
    ...
    def export_html_report(
        ...
        for r in self.results:
            ...
            rows_html += (
                f'<tr class="{cls}"><td>{r.event.event_id}</td>'
                f'<td>{r.event.category.value}</td>'
                f'<td>{r.event.description[:55]}{"…" if len(r.event.description) > 55 else ""}'
                f'{real_tag}</td>'
        ...
        for r in self.get_failures():
            snippet = json.dumps(r.event.log_data, indent=2)[:700]
            conds   = ", ".join(r.detection.matched_conditions[:3]) or "none matched"
            failures_html += (
                f'<div class="failure-card">'
                f'<h4>[{r.outcome}] {r.event.event_id}: {r.event.description}</h4>'
                f'<p><b>Category:</b> {r.event.category.value}'
                f' &nbsp;|&nbsp; <b>MITRE:</b> {r.event.attack_technique or "N/A"}</p>'
                f'<p><b>Notes:</b> {r.event.notes or "N/A"}</p>'
        ...
        for r in evasion_results:
            ...
            evasion_html += (
                ...
                f' <strong>{r.event.description}</strong>'
                f' <span class="ev-tags">{", ".join(r.event.tags or [])}</span>'
                f'{"<br><em>" + r.event.notes + "</em>" if r.event.notes else ""}'
                f'</div>\n'
            )
        ...
        for rec in recs:
            ...
            recs_html += (
                ...
                f'{rec.get("priority","info").upper()}</span> '
                f'<strong>{rec.get("title","")}</strong></div>'
                f'<p class="rec-body">{rec.get("body","")}</p>'
                f'<div class="rec-fix"><span style="color:{c}">FIX →</span> {rec.get("fix","")}</div>'
                f'</div>\n'
            )
...
<div class="meta">
  Rule: <strong>{self.engine.rule_name}</strong>
  &nbsp;·&nbsp; Generated: {now_str}
...
<div class="summary-row">
  ...
  <div class="si"><div class="sv">{m["evasion_resistance"]:.0%}</div><div class="sl">Evasion</div></div>
...
{"".join(
    ...
    for lbl, val, clr in [
        ...
        ("Evasion Resistance", m["evasion_resistance"] or 0, "#f59e0b"),
